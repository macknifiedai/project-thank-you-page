(function(){
  ...
  function run(){
    if(contactId){
      ...
      xhr.open('GET','https://app.macknified.com/api/landing/context/'+encodeURIComponent(contactId)+'?page_id=1580');
      xhr.onload=function(){
        if(xhr.status===200){
          try{
...
(function() {
      ...
      const get = (key) => decodeURIComponent(params.get(key) || 'Not provided');

      const firstName         = get('first_name');
      const email             = get('email');
      const phone             = get('phone');
      const company           = get('company');
      const selectedServices  = get('selected_services');
      const estimatedTotal    = get('estimated_total');
      const estimatedTimeline = get('estimated_timeline');
      const quoteBreakdown    = get('quote_breakdown');
      const projectDesc       = get('project_description');
      ...
      // Replace in h1 heading
      document.querySelectorAll('h1').forEach(h => {
          if (h.innerHTML.includes('{{first_name}}')) {
              h.innerHTML = h.innerHTML.replace('{{first_name}}', firstName);
          }
      });
  })();
...
(function(){
    var params = new URLSearchParams(window.location.search);
    var fields = ['first_name','email','phone','company','selected_services','estimated_total','estimated_timeline','quote_breakdown','project_description'];
    fields.forEach(function(key){
      var val = params.get(key);
      if(!val) return;
      document.body.innerHTML = document.body.innerHTML.split('{{'+key+'}}').join(val);
    });
  })();